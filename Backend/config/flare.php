...
return [
    ...
    */
    ...
    'censor' => [
        // Extended beyond the package default (password/password_confirmation
        // only) with this project's own sensitive fields — confirmed via
        // direct reading of the FormRequests/DTOs that actually accept them:
        // card_number/card_holder_name/expiry_month/expiry_year/cvv
        // (ProcessGatewayPaymentRequest — the demo payment gateway),
        // account_number (StorePaymentMethodRequest/UpdatePaymentMethodRequest
        // — bank account numbers, already encrypted at rest and masked in API
        // responses elsewhere in this codebase; must never leave the app raw
        // via an error report either), token (ResetPasswordRequest — a
        // credential-equivalent value).
        'body_fields' => [
            'password',
