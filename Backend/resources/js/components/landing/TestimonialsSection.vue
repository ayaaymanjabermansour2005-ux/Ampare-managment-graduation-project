...
<template>
  <section id="testimonials" class="py-14 sm:py-24 grid-texture section-tint-gold">
    <div class="max-w-6xl mx-auto px-4 sm:px-8">
      ...
      <div class="grid grid-cols-3 gap-2.5 sm:gap-5">
        <div
          ...
          :key="item.name + i"
          v-reveal="(i % 3) * 90"
          class="glass-card p-2.5 sm:p-6"
        >
          <div class="star-row flex gap-0.5 sm:gap-1 mb-2 sm:mb-3 text-[9px] sm:text-sm" :aria-label="t('common.rate_n_stars', { n: item.stars })" role="img">
            <Star
              v-for="s in 5"
              :key="s"
              aria-hidden="true"
          ...
          <p class="text-[10px] sm:text-[13px] text-[#555] dark:text-[#c8cac5] leading-relaxed mb-2 sm:mb-4 line-clamp-4 sm:line-clamp-none">"{{ item.text }}"</p>
          <div class="flex items-center gap-1.5 sm:gap-2">
            <div class="w-6 h-6 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br from-[#8A6D1F] to-[#52733D] flex items-center justify-center text-white text-[9px] sm:text-xs font-bold shrink-0">
              {{ item.name.slice(0, 1) }}
            </div>
            <div class="min-w-0">
              <p class="text-[9.5px] sm:text-[12px] font-bold truncate">{{ item.name }}</p>
              <p class="text-[8px] sm:text-[10px] text-[#6B6B6B] dark:text-[#a8aaa5] truncate">{{ item.role }}</p>
            </div>
          </div>
        </div>
      </div>
