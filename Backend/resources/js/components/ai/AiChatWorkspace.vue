...
import { ArrowLeft, ArrowRight, ArrowUp, Bot, Camera, Circle, CircleAlert, Ellipsis, FileText, LoaderCircle, MessagesSquare, Paperclip, Search, Send, Sparkles, SquarePen, X, ZoomOut } from "@lucide/vue";
    ...
    <div class="flex min-h-[calc(100vh-9.5rem)]">
      <aside class="w-full sm:w-80 shrink-0 flex flex-col border-e border-[#e7e2d6] dark:border-white/10 bg-[#fcfbf8]/70 dark:bg-white/[.02]" :class="{ 'hidden sm:flex': chat.activeSession.value || showComposer }">
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <div v-else-if="!filteredSessions.length" class="p-8 text-center text-[12px] text-[#8b8e88] dark:text-[#a8aaa5]"><ZoomOut class="text-2xl mb-3 block mx-auto" aria-hidden="true" />{{ t("ai_chat_workspace.no_matching_sessions") }}</div>
