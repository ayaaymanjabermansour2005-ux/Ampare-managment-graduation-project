...
const updatedAtLabel = computed(() =>
  t("landing.legal.privacy_policy.updated_at", {
...
const sections = computed(() =>
  Array.from({ length: 8 }, (_, i) => i + 1).map((n) => ({
    title: t(`landing.legal.privacy_policy.s${n}_title`),
    body: t(`landing.legal.privacy_policy.s${n}_body`),
  })),
);
</script>
...
<template>
  <div class="max-w-2xl mx-auto px-5 sm:px-8 py-16">
    ...
    <h1 class="text-[22px] sm:text-[26px] font-black mb-1.5">{{ t("landing.legal.privacy_policy.title") }}</h1>
    ...
    <p class="text-[13px] leading-relaxed mb-8">{{ t("landing.legal.privacy_policy.intro") }}</p>
